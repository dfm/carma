...
from ._carma import carma_log_likelihood, carma_covariance
...
class CARMAModel(object):
    ...
    def log_prior(self):
        if self.log_jitter < -12 or self.log_sigma < -12:
            return -np.inf
    ...
    def get_vector(self):
        return np.concatenate(([self.log_jitter, self.log_sigma],
                               self.arpars, self.mapars))

    def set_vector(self, vector):
        self.log_jitter = vector[0]
        self.log_sigma = vector[1]
        self.arpars = vector[2:2+self.p]
        self.mapars = vector[2+self.p:2+self.p+self.q]
